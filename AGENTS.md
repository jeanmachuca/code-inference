...
- **Stack Dockerfiles:** `src/services/api/Dockerfile` (python:3.12-alpine, uvicorn). `src/inference-vllm/`, `src/llama-stack/`, `src/ollama-stack/` are experimental stubs.
- **Inference container:** `ghcr.io/ggml-org/llama.cpp:server-cuda12-b9538`, starts with `--jinja --tools all --ctx-size ${CONTEXT_SIZE:-16384}`.

## OpenCode launch scripts

| Script | Use case |
|--------|----------|
| `./launch-opencode.sh` | Interactive OpenCode via `docker compose --profile tools run --rm opencode` |
| `./launch-fresh-opencode.sh` | Standalone container (no compose); pass `--full-isolation` for project-scoped volumes |
